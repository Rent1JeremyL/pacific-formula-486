package com.rent1.dao.auth;

import lombok.Data;

import com.rent1.utils.EncryptUtils;
import com.rent1.utils.URLUtils;

/**
 * The components of Voost's auth cookie
 * 
 * The cookie associated with a local identity looks like
 * "YOURID:EMAIL:EXPIRETIME:SIGNATURE". Expire time is millis since epoch. It
 * gets refreshed every time this login is executed (each page load). The cookie
 * is session scoped (maxAge 0). Note that there is javascript in master.coffee
 * that parses this cookie; be careful changing the format.
 */
@Data
public class AuthCookie {
	...
	/**
	 * Create a new cookie from the basic info, establishing expiration and
	 * signature.
	 */
	...
	/**
	 * This cookie as a string value which can be used with the String
	 * constructor
	 */
	...
	private String generateSig() {
		...
		return EncryptUtils.macHmacSHA256Hex(signIt, SECRET);
	}
