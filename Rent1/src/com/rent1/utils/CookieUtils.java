...
public class CookieUtils {
	...
	/**
	 * For systems like Facebook that encode a url-type map of values within a
	 * cookie.
	 * 
	 * @return null if cookie is not present
	 */
	public static Map<String, String> getSubCookies(HttpServletRequest request,
			String name) {
		Cookie cook = getCookie(request, name);
		if (cook == null) {
			return null;
			...
			for (String kvPair : kvpairs) {
				...
				cookMap.put(URLUtils.urlDecode(valPair[0]),
						URLUtils.urlDecode(valPair[1]));
			}
	...
	/**
	 * Sets a root cookie value with a specific maxAge in seconds (0 is delete,
	 * -1 is "nonpersistent")
	 */
	public static void setCookie(HttpServletResponse response, String name,
	...
	public static String toString(Cookie cook) {
		if (cook == null)
			...
			return String
					.format("Cookie{name=%s, value=%s, domain=%s, maxAge=%s, path=%s, secure=%s, version=%s, comment=%s}",
							cook.getName(), cook.getValue(), cook.getDomain(),
							cook.getMaxAge(), cook.getPath(), cook.getSecure(),
							cook.getVersion(), cook.getComment());
	}
}
